...
class Person {
	...
	eat(somefood) {
		this.stomach.length < 10 ? this.stomach.push(somefood) : this.stomach;
	}
	poop() {
...
class Lambdasian {
	constructor(myArgs) {
		(this.name = myArgs.name),
			(this.age = myArgs.age),
			(this.location = myArgs.location);
	}
	speak() {
...
class Instructor extends Lambdasian {
	...
	gradeStudent(student) {
		const getRamdomNum = () => Math.floor(Math.random() * 100);
		const addOrSubstract = getRamdomNum();
		const changeGrade = getRamdomNum();

		return addOrSubstract < 50
			? student.grade + changeGrade >= 100
				? (student.grade = 100)
				: (student.grade += changeGrade)
			: student.grade - changeGrade < 0
			? (student.grade = 0)
			: (student.grade -= changeGrade);
	}
}

const sebastian2 = new Instructor({
	name: "sebastian2",
	location: "OH",
	age: 29,
	specialty: "sales",
...
class Student extends Lambdasian {
	...
	graduate(instructorToGrade) {
		instructorToGrade.gradeStudent(this);
		...
		return `graduate from Lambda School`;
	}
}
